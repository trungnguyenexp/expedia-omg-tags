  ...
  (function(id, loader) {
    ...
    u.send = function (a, b) {
      if (u.ev[a] || u.ev.all !== undefined) {
        ...
        u.data.base_price = b['entity.hotels.results.results.0.rooms.0.averagePrice.currency'] + b['entity.hotels.results.results.0.rooms.0.averagePrice.amount']
        u.data.partner_id = '89';

        u.data.url +=  u.data.partner_id + "/?label=FH&guid=ON&script=0&ord=" + Math.floor((Math.random() * 10000000000000000));
        u.data.url += "&price=partner_id%3D" + u.data.partner_id + "%3B";
        u.data.url += "base_price_value_string%3D" + u.data.base_price + "%3B";
