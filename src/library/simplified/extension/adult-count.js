...
else if(utag.isPSR_HC() || utag.isPSR_FC() || utag.isPIS_HC() || utag.isPSR_FH_Responsive() || utag.isPIS_FH() || utag.isPSR_Mobile()){
    var roomsCount = b.entity.packageSearch.packageSearchParameters.travelers.length;
    var adults = 0;
    for(var i = 0; i < roomsCount; i++){
        var roomObject = "entity.packageSearch.packageSearchParameters.travelers." + i +".numberOfAdults";
...
/** old
 ...
 if(utag.isHSR() && typeof b.entity.hotels.search.hotelParameters.rooms != "undefined"){
    ...
    for(var i = 0; i < roomsCount; i++){
        var roomObject = "entity.hotels.search.hotelParameters.rooms." + i +".numAdults";
        adults = adults + parseInt(utag_data[roomObject]);
    }

    utag_data["adults"] = adults;
    utag.DB('Hotel Search page: Adult count =' + utag_data["adults"]);
    ...
    utag_data["adults"] = adults;
    utag.DB('Hotel IS page: Adult count =' + utag_data["adults"]);
}
    ...
    for (var i = 0; i < roomsCount; i++) {
        var roomObject = 'entity.packageSearch.packageSearchParameters.travelers.' + i + '.numberOfAdults';
        adults += parseInt(utag_data[roomObject]);
    }
    utag_data['adults'] = adults;
}
 else if(utag.isHCO() && typeof b.entity.checkout.hotel.rooms != "undefined"){
    ...
    utag_data["adults"] = adults;
    utag.DB('Hotel confirmation page: Adult count =' + utag_data["adults"]);
}

 //Flight path
 else if(utag.isFSR() || utag.isPSR_F_Responsive()){
    utag_data["adults"] = utag_data["entity.flightSearch.travelers.numberOfAdults"];
    utag.DB('Flight Search page: Adult count =' + utag_data["adults"]);
}
 else if(utag.isFRateDetails() ){
    utag_data["adults"] = utag_data["entity.flight.travelerInfo.numberOfAdults"];
    utag.DB('Flight rate details page: Adult count =' + utag_data["adults"]);
}
 else if (utag.isPRateDetails() && utag_data['entity.tripDetails.travelersInfo.numberOfAdults'])
 {
     utag_data["adults"] = utag_data['entity.tripDetails.travelersInfo.numberOfAdults'];
 }
 else if(utag.isFPymt() || utag.isFCO() || utag.isPPymt() || utag.isPCO()){
    utag_data["adults"] = utag_data["entity.checkout.flightOffer.travelersInfo.numberOfAdults"];
    utag.DB('Flight/Package payment/checkout page: Adult count =' + utag_data["adults"]);
}
 ...
 //Package path
    ...
    utag_data["adults"] = adults;
    utag.DB('Package Search page: Adult count =' + utag_data["adults"]);
}
 else if(utag.isPSR_HC() || utag.isPSR_FC() || utag.isPIS_HC() || utag.isPSR_FH_Responsive() || utag.isPIS_FH() || utag.isPSR_Mobile()){
    var roomsCount = b.entity.packageSearch.packageSearchParameters.travelers.length;
    var adults = 0;
 ...
 else if (utag.isLXCO())
 {
     var adultCount = 0;
     var seniorCount = 0;
     if (!isNaN(utag_data['entity.checkout.activity.travelerInfo.numberOfAdults']))
     {
         adultCount = parseInt(utag_data['entity.checkout.activity.travelerInfo.numberOfAdults']);
     }
     if (!isNaN(utag_data['entity.checkout.activity.travelerInfo.numberOfSeniors']))
     {
         seniorCount = parseInt(utag_data['entity.checkout.activity.travelerInfo.numberOfSeniors']);
     }
     utag_data['adults'] = adultCount + seniorCount;
 }
 else if (utag.isCruiseCO())
 {
     var adultCount = 0;
     var seniorCount = 0;
     if (!isNaN(utag_data['entity.checkout.cruise.traveler.numberOfAdults']))
     {
         adultCount = parseInt(utag_data['entity.checkout.cruise.traveler.numberOfAdults']);
     }
     if (!isNaN(utag_data['entity.checkout.cruise.traveler.numberOfSeniors']))
     {
         seniorCount = parseInt(utag_data['entity.checkout.cruise.traveler.numberOfSeniors']);
     }
     utag_data['adults'] = adultCount + seniorCount;

 }
 else if (utag.isMCO()) {
  if (utag_data["entity.checkout.flightOffers.0.travelersInfo.numberOfAdults"] != undefined) {
    utag_data["adults"] = utag_data["entity.checkout.flightOffers.0.travelersInfo.numberOfAdults"];
  }
